...
select pizza_category, sum(total_price) as total_sales ,(sum(total_price)   / (select sum(total_price) from pizza_sales where month(order_date)=1 
 )  ) * 100   as pct from pizza_sales
 ...
 select pizza_size , round(sum(total_price)  * 100  / ( select sum(total_price) from pizza_sales where quarter(order_date) = 1 ) ,2 )  as persale_by_size
 
 from pizza_sales where quarter(order_date) = 1  group by pizza_size order by sum(total_price) desc;
 
 
 
 
 
 select sum(total_price) as  total_revenue,sum(quantity) as total_quantity, count(distinct order_id) as  total_order from pizza_sales;
 
 
 #
 
 
--  select total_price , quantity ,  count( distinct order_id )from pizza_sales  group by  order by total_price , quantity  desc ;

#pizza name , revenue , by quantity , total quantity , orders  by asc  and dsc


#by revenue asc bottom 5 
...
# by pizza_quantity 
select  pizza_name , sum(quantity) as quantity from pizza_sales group by pizza_name order by sum(quantity) asc limit 5; 
...
select pizza_name , sum(quantity) as  quantity from pizza_sales group by pizza_name order by sum(quantity) desc limit 5;
...
select pizza_name,count(distinct order_id ) as total_ordrs from pizza_sales 
group by pizza_name order by total_ordrs desc limit 5;
